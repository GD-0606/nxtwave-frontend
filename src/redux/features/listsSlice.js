...
export const listsSlice = createSlice({
  ...
  reducers: {
    ...
    startListCreation(state) {
      const first = state.lists.filter(
        (item) => item.list_number === state.selectedLists[0]
      );
      const second = state.lists.filter(
        (item) => item.list_number === state.selectedLists[1]
      );
    ...
    moveItem(state, action) {
      ...
      const item = state.tempLists[fromIndex].lists.find(
        (i) => i.id === itemId
      );
      state.tempLists[fromIndex].lists = state.tempLists[
        fromIndex
      ].lists.filter((i) => i.id !== itemId);
